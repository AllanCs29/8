...
fun StudyTab() {
    ...
    ElevatedCard(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(if (onWork) "Sesión de Trabajo" else "Descanso",
                    fontWeight = FontWeight.SemiBold, modifier = Modifier.weight(1f))
                val mins = remaining / 60; val secs = remaining % 60
                Text("%02d:%02d".format(mins, secs), fontWeight = FontWeight.Bold)
            }
            Spacer(Modifier.height(8.dp))
    ...
    Canvas(Modifier.fillMaxWidth().height(160.dp)) {
        if (last14.size >= 2) {
            ...
            for (i in 1 until last14.size) {
                ...
                // Gris suave en lugar de verde para evitar confusiones visuales
                drawLine(Color(0xFF9E9E9E), Offset(px, py), Offset(x, y), strokeWidth = 6f)
                px = x; py = y
            }
        }
    }
